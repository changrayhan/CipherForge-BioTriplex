...
class PartyM:
    ...
    def _setup_bfv(self, bfv_sk_pem: bytes, bfv_pk_pem: bytes) -> None:
        ...
        self.bfv_backend._encryptor = Encryptor(
            self.bfv_backend._context, self.bfv_backend._public_key,
        )
    ...
    def backward_and_update(self, payload: Dict) -> Dict:
        ...
        plain_modulus = get_plain_modulus(
            int(self.bfv_backend.poly_degree), int(self.bfv_backend.plain_bits)
        )
        ...
        for local_i, row in enumerate(rows):
            ...
            if self.config.get("debug_grad") and local_i == 0:
                logger.warning(
                    "debug_grad step=%d row=%d masked_int[:4]=%s s_share[:4]=%s "
                    "diff_mod[:4]=%s centered[:4]=%s",
                    step, row,
                    masked_int[:4].tolist(), s_arr[:4].tolist(),
                    diff_mod[:4].tolist(), diff_centered[:4].tolist(),
                )
        ...
        return {
            ...
            "g_absmax": float(np.abs(g_accum).max()) if g_accum.size else 0.0,
            "g_meanabs": float(np.abs(g_accum).mean()) if g_accum.size else 0.0,
