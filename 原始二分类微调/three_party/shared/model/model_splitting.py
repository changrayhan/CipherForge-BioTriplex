...
class _LlamaUShard(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, position_ids=None):
        ...
        # FlashAttention2 需要 2D mask（HF 内部再转 4D）；传 4D 加法掩码会报错
        use_fa2 = getattr(self.config, "_attn_implementation", "sdpa") == "flash_attention_2"
        layer_mask = mask_2d if use_fa2 else mask_4d
        ...
        for layer in self.layers:
            if use_gc:
                ...
                layer_out = torch.utils.checkpoint.checkpoint(
                    layer.forward,
                    hidden,
                    layer_mask,  # attention_mask (4D additive)
                    position_ids,
