...
def _letter_to_coarse(letter: str) -> Optional[str]:
    ...
    """
    try:
        from shared.data.biotriplex_dataset import GENERAL_RELATIONS
    except Exception:
        return None
    ...
    def _collate(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for k in keys:
            ...
            if isinstance(vals[0], torch.Tensor):
                ...
                # Handle nested list (e.g. output_ids from ClinVarQADataset).
                # Must become a tensor so that .to("cpu") / .shape / .gather()
                # work in remote_val() and _run_val_clinvar().
                try:
...
class Trainer:
    ...
    def _maybe_fit_cti_for_epoch(self, train_loader: Any) -> None:
        ...
        if priv is None or not hasattr(priv, "fit_cti"):
            return
        task_type = str(getattr(self.config, "task_type", "classification"))
        # CTI applies to every classification-like task (incl. clinvar /
        # biotriplex7 / biotriplex21 loaded through the JSONL QA datasets).
        ipc_cfg = getattr(getattr(self, "ipc", None), "config", None) or {}
        if task_type != "classification" and not bool(ipc_cfg.get("answer_token_to_class")):
            return
        try:
    ...
    def _derive_class_idx_from_batch(self, batch: Dict) -> Optional[List[int]]:
        """Extract coarse class indices from a batch.

        Prefers ``label_idx`` (set by :class:`BioTriplexQADatasetClassification`).
        Then maps the discriminating answer token (e.g. the letter of " j)")
        through the protocol config's ``answer_token_to_class``, which covers
        every JSONL QA task (clinvar / biotriplex7 / biotriplex21).
        Falls back to decoding the first non-pad label token and looking it up
        in :data:`GENERAL_REL_TO_IDX` (7-class GenRel mapping).
        ...
        # ---- answer_token_to_class path (generic JSONL QA tasks) ----
        try:
            ipc_cfg = getattr(getattr(self, "ipc", None), "config", None) or {}
            tok2cls = ipc_cfg.get("answer_token_to_class") or {}
            output_ids = batch.get("output_ids")
            if tok2cls and isinstance(output_ids, torch.Tensor):
                out: List[int] = []
                for row in output_ids:
                    ...
                    if valid.numel() == 0:
                        ...
                        continue
                    toks = valid.tolist()
                    # Answer format " j)" -> [blank, letter, ")"], " Yes" ->
                    # [blank, Yes]; the discriminating token is the 2nd token.
                    tok = toks[1] if len(toks) >= 2 else toks[0]
                    out.append(int(tok2cls.get(str(tok), -1)))
        ...
        try:
            from shared.data.biotriplex_dataset import GENERAL_REL_TO_IDX
            import torch as _torch
    ...
    def _run_val_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        val_loader = DataLoader(
            ...
            shuffle=True,
            num_workers=0,
            generator=torch.Generator().manual_seed(int(self.config.seed) + epoch),
            collate_fn=collate,
        )
    ...
    def _log_epoch(self, epoch: int, metrics: Dict) -> None:
        ...
        # ClinVar path: show baseline-aligned metrics (AUPRC/AUC/acc/per-gene).
        if str(getattr(self.config, "task_type", "")) == "clinvar":
            logger.info(
                "Epoch %d: train_loss=%.4f | val_ce_loss=%.4f | "
                "val_AUPRC=%.4f | val_AUC=%.4f | val_acc@0.5=%.4f | "
                "val_per_gene_AUPRC=%.4f (n_genes=%d) | val_samples=%d",
                epoch,
                metrics.get("train_loss", 0),
                metrics.get("val_ce_loss", 0),
                metrics.get("val_auprc", 0),
                metrics.get("val_auc", 0),
                metrics.get("val_accuracy", 0),
                metrics.get("val_per_gene_mean_auprc") or 0.0,
                int(metrics.get("val_per_gene_n_genes") or 0),
                int(metrics.get("val_samples") or 0),
            )
        else:
            logger.info(
