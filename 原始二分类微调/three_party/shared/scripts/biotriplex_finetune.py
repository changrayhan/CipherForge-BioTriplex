...
from shared.data.biotriplex_dataset import build_biotriplex_dataset  # noqa: E402
from shared.training.trainer import Trainer, TrainerConfig  # noqa: E402
...
def run_stage0(args, logger_: logging.Logger) -> Dict[str, Any]:
    ...
    if not args.skip_db:
        ...
        from shared.scripts.build_encrypted_db import build_encrypted_db
    ...
    if not args.skip_hints:
        ...
        from shared.scripts.build_s3pir_hints import build_s3pir_hints
...
def run_stage1(args, logger_: logging.Logger) -> Dict[str, Any]:
    ...
    from shared.parties.heterogeneous_protocol import HeterogeneousProtocol
...
def save_peft_adapter(protocol, base_model_path: str, adapter_dir: str, logger_: logging.Logger) -> None:
    ...
    if not lora_state:
        raise RuntimeError(
            "save_peft_adapter: M checkpoint contains no LoRA tensors — "
            "training did not actually update any LoRA weights, refusing to "
            "export a degenerate adapter."
        )
    ...
    u_layers = None
    # RemoteProtocol exposes ``u_layers`` directly; the in-process protocol
    # exposes it on ``party_m.model`` / ``party_m.spec``.
    _pm = getattr(protocol, "party_m", None)
    for cand in (protocol, getattr(_pm, "model", None), getattr(_pm, "spec", None)):
        try:
            _u = getattr(cand, "u_layers", None)
            if _u is not None:
                u_layers = int(_u)
                break
        except Exception:
    ...
    if u_layers is None:
        raise RuntimeError(
            "save_peft_adapter: cannot resolve u_layers offset from the protocol "
            "(set protocol.u_layers for the three-party coordinator)"
        )
...
def _patch_trainer_for_max_steps(trainer, max_steps: int, logger_) -> None:
    ...
    def patched(epoch: int):
        if trainer.global_step >= max_steps:
            logger_.info(
                "[max_train_steps] global_step=%d already >= %d — skipping epoch %d training",
                trainer.global_step, max_steps, epoch,
            )
            # Still run validation so the epoch logs real metrics (not zeros).
        ...
        # Mirror the base _run_epoch: fit the per-class CTI on the first batch.
        if bool(getattr(trainer.config, "dp_enable", False)):
            trainer._maybe_fit_cti_for_epoch(loader)
        total = 0
        loss_sum = 0.0
        ce_sum = 0.0
        ce_count = 0
        ...
        try:
            ...
            # 随机抽取（而不是取前 512 条）：val 数据未打乱时前 N 条可能
            # 全是同一类，导致小批量验证 AUPRC 恒为 0 的假象。
            _gen = _torch.Generator().manual_seed(int(getattr(trainer.config, "seed", 42)) + epoch)
            _idx = _torch.randperm(len(trainer.val_ds), generator=_gen)[:n_val].tolist()
            trainer.val_ds = _Subset(trainer.val_ds, _idx)
            val_metrics = trainer._run_val_epoch(epoch)
